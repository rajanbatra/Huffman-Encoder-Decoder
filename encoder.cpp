...
#include <queue>
using namespace std;
void FillFreqMapFromFile(	std::ifstream &infile,std::unordered_map<int,unsigned int> &freq_map){
...
void CreateHnodeVecFromFreqMap(	std::unordered_map<int,unsigned int> const &freq_map,	std::vector<hnode const*> &hnode_vec){
...
hnode const* BuildTreeFheap(std::unordered_map<int,unsigned int> const &freq_map){
...
fheap fheap_obj(hnode_vec);
	...
	while (fheap_obj.size() > 4)
	{
		...
		hnode const *left =fheap_obj.fheap::get_min();
...
hnode const* BuildTreeBheap(	std::unordered_map<int,unsigned int> const &freq_map){
...
hnode const* BuildTreePheap(	std::unordered_map<int,unsigned int> const &freq_map){
...
void printCodes(	hnode const *root,	std::string str,	std::unordered_map<int,std::string> &code_table_map	)
{
...
void binfilecreate(	std::unordered_map<int,std::string> &code_table_map,	char *filename,	char const *out	){
